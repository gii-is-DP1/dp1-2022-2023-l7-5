...
//		
//		mockMvc.perform(get("/games/new").with(csrf())).andExpect(model().attributeHasErrors("game"))
...
//		.andExpect(view().name("games/createOrUpdateGameForm")).andExpect(model().attributeExists("game"));
//		
//		
//	}
...
//		
//		mockMvc.perform(get("/games/{id}", TEST_ID)).andExpect(status().isOk())
//			.andExpect(model().attribute("game", hasProperty("mode", is("SURVIVAL")))).andExpect(view().name("games/gameDetails"));
//	}
